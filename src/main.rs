...
fn main() {
    let matrix: Matrix<Complex<f64>> = Matrix::new(
        vec![
...
impl<T: Num + Copy> Matrix<T> {
    ...
    pub fn row_indices(&self) -> Vec<Vec<usize>> {
    ...
    pub fn col_indices(&self) -> Vec<Vec<usize>> {
...
pub mod tests {
    ...
    pub fn test_rows_and_cols() {
        ...
        assert_eq!(m1.row_indices(), vec![vec![0, 1, 2], vec![3, 4, 5]]);
        assert_eq!(m1.col_indices(), vec![vec![0, 3], vec![1, 4], vec![2, 5]]);
        ...
        m1.transpose();
        assert_eq!(m1.row_indices(), vec![vec![0, 3], vec![1, 4], vec![2, 5]]);
        assert_eq!(m1.col_indices(), vec![vec![0, 1, 2], vec![3, 4, 5]]);
        assert_eq!(
            m1.rows(),
            vec![vec![1.0, 4.0], vec![2.0, 5.0], vec![3.0, 6.0]]
        );
